...
def distance_counter(img, founded_people, top, q, h, plot_box, det, save_img, b, draw):
    ...
    cv2.putText(img, f"Total Object: = {len(det)}", (0, 255), cv2.FONT_HERSHEY_TRIPLEX, 1, (0, 0, 0), 1)


# def plot_circle(image,m,j,founded_people,radius=1,color=(0,0,255),line_thickness=3):

def plot_circle(image, m, j, founded_people, color=(0, 0, 255), line_thickness=3):
    tl = line_thickness or round(0.002 * (image.shape[0] + image.shape[1]) / 2) + 1
    c1 = (int(founded_people[m][0]), int(founded_people[m][1]))
    # k1, k2 = (int(founded_people[j][0]), int(founded_people[j][3])), (
    #     int(founded_people[j][2]), int(founded_people[j][3]))

    cv2.circle(image, center=c1, radius=2, color=color, thickness=tl, lineType=cv2.LINE_AA)

    # cv2.circle(image, center=(k2),radius=1, color=color, thickness=tl, lineType=cv2.LINE_AA)


def save(im0, l, b):
    bb = im0[int(l[1]):int(l[3]), int(l[0]):int(l[2])]
    path = "/home/nexgits/YOLO/yolov7/CloseCount"
...
def count(classes, image):
    ...
    for i, (k, v) in enumerate(classes.items()):
        ...
        cv2.putText(image, str(a), (int(align_right), align_bottom), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1,
                    cv2.LINE_AA)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                if a == True:
                    distance_counter(im0, founded_people, top, q, h, plot_circle, det, save_img, b, draw)
